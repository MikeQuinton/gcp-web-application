...
resource "google_compute_router" "app-router" {
  name    = "${var.network_name}-router-prod"
...
resource "google_compute_router_nat" "app-nat" {
  project = var.project
  name    = "${var.network_name}-nat-prod"
...
resource "google_compute_firewall" "http" {
  project     = var.project
  name        = "${var.network_name}-http-allow-prod"
  network     = google_compute_network.app-network.name
...
resource "google_compute_firewall" "ssh" {
  project     = var.project
  name        = "${var.network_name}-iap-allow-prod"
  network     = google_compute_network.app-network.name
...
resource "google_compute_security_policy" "sec-policy" {
  ...
  rule {
    ...
    match {
      ...
      config {
        src_ip_ranges = ["*"] # This could be a range of public IP's used by the client
      }
    }
...
module "lb-http" {
  ...
  backends = {
    default = {
      ...
      enable_cdn                      = false
      security_policy                 = google_compute_security_policy.sec-policy.id
